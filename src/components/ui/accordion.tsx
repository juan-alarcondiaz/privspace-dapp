...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn(
      "flex flex-col gap-4 rounded-2xl border border-jaguar-800 bg-victoria-950 text-left",
      className,
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "p-4 flex flex-1 items-center justify-between gap-3 text-left font-medium " +
          "transition-all",
        className,
      )}
      {...props}
